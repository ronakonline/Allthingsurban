              ...
              <td class="text-center">
                ...
                if($expired_date >= strtotime(date('dMY'))){ ?>
                  <?php if($booking['status'] == 0){ ?>
                    <a href="<?php echo site_url('admin/booking_request_restaurant/approved/'.$booking['id']); ?>" class="btn btn-default btn-sm btn-icon icon-left">
                      <i class="entypo-check"></i>
                      <?php echo get_phrase('approve'); ?>
                    </a>
                  <?php }else{ ?>
                    <a href="<?php echo site_url('admin/booking_request_restaurant/pending/'.$booking['id']); ?>" class="btn btn-default btn-sm btn-icon icon-left">
                      <i class="entypo-check"></i>
                      <?php echo get_phrase('pending'); ?>
                    </a>
                  <?php } ?>
                <?php } ?>
                <a href="#" class="btn btn-danger btn-sm btn-icon icon-left" onclick="confirm_modal('<?php echo site_url('admin/booking_request_restaurant/delete/'.$booking['id']); ?>');">
